...
use Imagick;
use ImagickPixel;
use Lychee\Support\PermissibleFile;
...
final class Photo {
	...
	public function add(array $files, $albumID = 0, $returnOnError = false) {
		// Check permissions
		if (!(new PermissibleFile(LYCHEE_UPLOADS))->isReadWritePermitted() ||
			!(new PermissibleFile(LYCHEE_UPLOADS_BIG))->isReadWritePermitted() ||
			!(new PermissibleFile(LYCHEE_UPLOADS_THUMB))->isReadWritePermitted()
		) {
	...
	private function createMedium($url, $filename, $width, $height) {
		...
		// Check permissions
		if (!(new PermissibleFile(LYCHEE_UPLOADS_MEDIUM))->isReadWritePermitted()) {
